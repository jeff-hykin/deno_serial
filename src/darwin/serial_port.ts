...
export class SerialPortDarwin implements SerialPort, AsyncDisposable {
    ...
    async open(options: SerialOptions) {
        ...
        if (options.parity && options.parity !== "none" && options.parity !== "even" && options.parity !== "odd") {
            throw new TypeError("Invalid parity, must be one of: none, even, odd")
        }
        this.options = options

        const shouldCreate = 0
        const fd = await this._darwinLibc.open(cString(this.name), Number(O_RDWR | O_NOCTTY | O_NDELAY), shouldCreate)
        unwrap(fd)
        this._fd = fd
        ...
        var termiosStruct = new Uint8Array([...iflagAsArray, ...oflagAsArray, ...cflagAsArray, ...lflagAsArray, ...ccAsArray, ...unknownAsArray, ...ispeedAsArray, ...ospeedAsArray])
        // set: baudRate
        ...
        var termiosStruct = new Uint8Array([...bigIntTo64IntLittleEndianBytes(iflag), ...bigIntTo64IntLittleEndianBytes(oflag), ...bigIntTo64IntLittleEndianBytes(cflag), ...bigIntTo64IntLittleEndianBytes(lflag), ...cc, ...unknown, ...ispeed, ...ospeed])
