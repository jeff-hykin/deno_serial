...
function getPortName(devInfoList: Deno.PointerValue, devInfoData: Uint8Array) {
    const hkey = Di.SetupDiOpenDevRegKey(devInfoList, devInfoData, Di.DICS_FLAG_GLOBAL, 0, Di.DIREG_DEV, Reg.KEY_READ)
    const nameBuf = new Uint8Array(256)
    const len = new Uint32Array([nameBuf.byteLength])
    const result = Reg.RegQueryValueExA(hkey, "PortName", null, null, nameBuf, new Uint8Array(len.buffer))
    unwrap(result)
    const nameLen = len[0]
...
function getPortInstanceId(devInfoList: Deno.PointerValue, devInfoData: Uint8Array) {
    const instanceIdBuf = new Uint8Array(260)
    const instanceIdLen = new Uint32Array(1)
    unwrap(Di.SetupDiGetDeviceInstanceIdA(devInfoList, devInfoData, instanceIdBuf, instanceIdBuf.byteLength, new Uint8Array(instanceIdLen.buffer)))
    return new TextDecoder().decode(instanceIdBuf.slice(0, instanceIdLen[0] - 1))
}

function getPortProperty(devInfoList: Deno.PointerValue, devInfoData: Uint8Array, id: number) {
    const resultBuf = new Uint8Array(260)
    const requiredSize = new Uint32Array(1)
    Di.SetupDiGetDeviceRegistryPropertyA(devInfoList, devInfoData, id, null, resultBuf, resultBuf.byteLength, new Uint8Array(requiredSize.buffer))
...
export function SetupDiClassGuidsFromNameA(className: string) {
    ...
    const result = Di.SetupDiClassGuidsFromNameA(className, guidList, guidListSize, actualSizePtr)
    unwrap(result)
    ...
    for (const guid of SetupDiClassGuidsFromNameA("Ports")) {
        ...
        for (let i = 0; true; i++) {
            ...
            if (!result) {
                ...
                const instance = instanceId.match(/(\w+)\\VID_([A-Fa-f0-9]+)&PID_([A-Fa-f0-9]+)\\([A-Fa-f0-9]+)/)
                ...
                const manufacturer = getPortProperty(devInfoList, devInfoData, 0x0000000b)
                const friendlyName = getPortProperty(devInfoList, devInfoData, 0x0000000c)
